...
- **FR-009**: El sistema DEBE permitir cerrar un equipo fijando una fecha de fin que se propaga a
  ...
  crear un nuevo equipo con una nueva fecha de inicio (los equipos no llevan nombre — ver FR-005).
...
- **FR-022**: Las operaciones de escritura sobre `ClientTeam` y `ClientAssignment` MUST aplicar control
  de concurrencia optimista basado en una columna `version` (smallint, monotónica, incrementa en cada
  UPDATE — ver ADR-0010 en `decisions.md`). El cliente envía la `version` que tenía al cargar el equipo
  vía header `If-Match: <version>`; si la BD tiene una versión mayor, el backend rechaza con HTTP 409
  y la UI muestra un aviso *"El equipo ha cambiado, recarga para ver el estado más reciente"* sin
  perder los datos introducidos. *(Se descartó `updatedAt` por colisiones a nivel milisegundo — PO
  ratification 2026-06-01.)*

### Key Entities

- **Equipo** (`Team`, impl. `ClientTeam`): Agrupación de personas con un responsable, una fecha de
  inicio y opcionalmente una fecha de cierre. **No lleva campo `name` persistido** (PO 2026-06-01 —
  ver FR-005). Estado calculado: `incomplete` | `active` | `closed` (ver FR-003 y FR-009).
- **Miembro del Equipo** (`TeamMember`, impl. `ClientAssignment`): Persona que pertenece al equipo,
  con rol (responsable, coordinador, asesor, técnico), porcentaje de carga, fecha de inicio y fecha
  de fin opcional. Cuando `role = asesor`, el atributo booleano `isPrimaryAdvisor` indica si es el
  **asesor principal** por (cliente, departamento) — exactamente uno activo por departamento (ver
  FR-005 y Clarifications 2026-05-29).
...
| # | Tema | Decisión PO |
|---|---|---|
| 1 | Suma 100% del equipo | **Dos coberturas** (asesores 100% + técnicos 100% por separado), agregando entre todos los equipos del **departamento** del cliente (no por equipo individual). PO 2026-06-01. |
...
- Q (post-sesión) · Modelo de suma 100% → A: **Dos coberturas independientes (asesores 100% + técnicos 100%) y suma POR DEPARTAMENTO del cliente, NO por equipo individual**. Confirmado por la PO con el frame `08-multi-equipo/01-multi-equipo-fiscal-larsa-costa.png`: 2 equipos Fiscal con 3 asesores totales distribuidos entre ambos, todos sumando 100% en conjunto. Si hay 4 asesores en 2 equipos Fiscal, cada uno al 25% → 100%. Lo mismo con técnicos por separado. **Esto invalida ADR-0008 (single cobertura por equipo)** — la decisión debe registrarse como nuevo ADR que supersede al anterior. **Cambia FR-003** (aplicado en esta misma iteración).
...
- Conflict flag pendiente: la decisión "dos cubos separados (asesores 100% + técnicos 100%)" tomada en una iteración paralela del 2026-06-01 mañana **queda invalidada** — la decisión madura del 2026-05-28 (un solo cubo, ratificada en ADR-0008) prevalece. Spec actual mantiene **single cobertura**.
