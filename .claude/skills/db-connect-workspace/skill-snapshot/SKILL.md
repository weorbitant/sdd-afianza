...
| `pd-service-data-factory` | `data-factory` | `pd-service-data-factory-{env}` *(verify in .env)* |
| `pd-service-jira-adapter` | `jira-adapter` | `pd-service-jira-adapter-{env}` *(verify in .env)* |
...
|-------------|------|----------------------|
| local | `postgresql` | `Password123` (standard dev default) |
| dev | `postgres` | Service `.env` → `POSTGRES_PASSWORD` |
| prod | `postgres` | Service `.env` (commented prod block) |
...
plataforma-del-dato/pd-service-obligations-api/.env-sample
plataforma-del-dato/pd-service-data-factory/.env   (if exists)
```
